...
module.exports = function() {
    ...
    return function(model) {
        ...
        domMgr.mk('h2',wrapper,function() { return this.tr((({ key:"Automation" }))); });
        domMgr.mk('p',wrapper,function() { return this.tr((({ key:"Igaro App supports automated authentication transparent to code (which needs no handling) and the user (no URL change or redirection)." }))); });
        domMgr.mk('p',wrapper,function() { return this.tr((({ key:"A service which requires credentials emits a HTTP 401 which is captured and invokes an OAuth2 mechanism. Once complete the previous request and any that have came in since are then actioned." }))); });

        domMgr.mk('h3',wrapper,function() { return this.tr((({ key:"Example" }))); });
        domMgr.mk('p',wrapper,function() { return this.tr((({ key:"The code below adds automated credential handling to the app. An entry for <i>google</i> has been added, all that requires adding is the developer id and other details described in the <b>instance.oauth2</b> documentation." }))); });
...
events.on('instance.xhr','response', function(p) {\n\
    ...
    return auth.set().then(function() {\n\
        ...
        domMgr.mk('h1',wrapper,function() { return this.tr((({ key:"CORS" }))); });
        domMgr.mk('p',wrapper,function() { return this.tr((({ key:"<b>instance.xhr</b> supports CORS (browser dependent)." }))); });
        domMgr.mk('p',wrapper,null,function() {

            domMgr.mk('button',this,function() { return this.tr((({ key:"Next Chapter - Design" }))); }).addEventListener('click',function() {
