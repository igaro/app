module.exports = function (app,passobj) {
    ...
    var workers = new Array({ uid:-1, done:true, module: { name:'instance.amd.js' }});
    var loaded = new Array();

    var promise = function(self) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                self.modules.forEach(function (m) {
                    ...
                    if (! m.requires) m.requires = new Array();
                    var wk, n=m.name;
    ...
    var setBits = function(p) {
      if (p.modules) this.modules = p.modules;
      if (p.repo) this.repo = p.repo;
      if (p.onProgress) this.onProgress = p.onProgress;
    }

    var amd = function(o) {
        var self = this;
        var repo = this.repo = passobj.repo;
        if (o) setBits.call(this,o);
    ...
    var worker = function(o) {
        ...
        var modname = this.module.name;
        var e = /^.+\.([^.]+)$/.exec(modname.toLowerCase());
        var type=this.type = e === null? '' : e[1];
        var file = this.file = mod.repo+(mod.nosub? '' : '/'+type+'/')+modname;
        var uid = this.uid = Math.floor((Math.random() * 9999));
    ...
    worker.prototype.run = function() {
        if (this.done) return this.runHandlers(0);
        ...
        var mod = this.module;
        var self = this;
        var fxhr = { xhr:xhr, res:file };
        xhr.onreadystatechange = function() {
            if (xhr.readyState !== 4) return;
            if (xhr.status === 200 || (xhr.status === 0 && xhr.responseText)) {
                if (type === 'js') {
                    var module = {};
                    module.requires = new Array();
                    module.exports = null;
