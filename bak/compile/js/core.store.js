        ...
        set : function (name,id,value,o) {
            ...
            if (type === 'cookie') {
                ...
                if (type === 'local') {
                    ...
                    try { 
                        return sessionStorage.setItem(uid,value); 
                    } 
                    catch (e) { 
                        return e; 
                    }
        ...
        get : function (name,id,o) {
            ...
            if (type === 'cookie') {
                ...
                while ((j < document.cookie.length) && done === false) { 
                    ++j;
                    if (document.cookie.substring(j, j + oid.length) !== oid) 
                        continue;
                    k=0;  
                    x='';
                    while (x !== '' && x !== ';') { 
                        ++k; 
                        x = document.cookie.substring(j + oid.length + k, j + oid.length + k - 1); 
                    }
                ...
                try { 
                    ...
                    debug.log.append({ module:'core.store', action:'get', error:e }); 
                }
            }
            return null;
        }
